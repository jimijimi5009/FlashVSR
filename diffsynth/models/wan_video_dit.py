...
def flash_attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, num_heads: int, compatibility_mode=False, attention_mask=None, return_KV=False):
    if attention_mask is not None and BLOCK_ATTN_AVAILABLE and USE_BLOCK_ATTN:
        seqlen = q.shape[1]
        seqlen_kv = k.shape[1]
    ...
    elif attention_mask is not None and SAGE_ATTN_AVAILABLE and not USE_BLOCK_ATTN:
